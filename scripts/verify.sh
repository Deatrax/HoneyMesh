...
PERMABLOCK_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X PATCH \
  -H "Authorization: Bearer ${ANALYST_TOKEN}" -H "Content-Type: application/json" \
  -d '{"version":0}' \
  "http://localhost:8080/api/incidents/999999/perma-block")
[ "$PERMABLOCK_STATUS" = "403" ] || [ "$PERMABLOCK_STATUS" = "401" ]
check "analyst (non-admin) token is blocked on the admin-only perma-block endpoint (got $PERMABLOCK_STATUS)" $?
